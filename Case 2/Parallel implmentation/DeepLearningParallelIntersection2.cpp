...
void parallelIntersectionDeepLearning(const char* inPath1, const char* inPath2, char* outPath, char* gridImageDir, int gridDimX, int gridDimY)
{
	...
	if (!rank)
	{
		...
		for (int i = 0; i < gridDimX * gridDimY; i++) gridPointNum[i] = 0; //memset(gridPointNum, 0, sizeof(gridPointNum));
		...
		cout << "Slave proceses are calculating features of each grid..." << endl;
		for (int i = 0; i < size - 1; i++)
		{
		...
		for (int i = 0; i < size - 1; i++)
		{
			...
			const char* function = "alexnet";
			const char* modelPath = "/home/fgao/LoadBalance/Intersection/python/ml_env/dl/intersection/result_model/alexnet1.pth";
			const char* inputRankDir = inputRankDirStr.c_str();
			const char* outputCIPath = (inputRankDirStr + "/predictCI.txt").c_str();
			string gpuIndexStr;
			gpuIndexStr = "cuda:0"; //use gpu
			gpuIndexStr = "N"; //use cpu
		...
		cout << "cnn model calling time cost is " << predictCost << endl;
	...
	if (rank)
	{
		...
		cout << rank << ": the data from master thread " << "was received for calculating features" << endl;
		...
		time_t processRasterizeBegin, processRasterizeEnd;
		time(&processRasterizeBegin);
		for (int k = startGridRange; k < endGridRange; k++)
		{
			...
			string sampleImagePathStr = inputRankDirStr + "/" + indexStr + ".tif";
			...
			OGRLayer* poSampleLayer = pSampleDS->CreateLayer(indexStr.c_str(), poSpatialRef, wkbMultiLineString, NULL);
			...
			for (int j1 = 0; j1 < geoGridCI[k - startGridRange]->polygonsFID1.size(); j1++)
			{
				...
				OGRErr error = poSampleLayer->CreateFeature(poSampleFeature);
		...
		time(&processRasterizeEnd);
		double processRasterizeCost = difftime(processRasterizeEnd, processRasterizeBegin);
		cout << "process " << rank << ": rasterizing time cost is " << processRasterizeCost << endl;
		
		removeFile((inputRankDirStr + "/").c_str(), "shp");
		removeFile((inputRankDirStr + "/").c_str(), "shx");
		removeFile((inputRankDirStr + "/").c_str(), "dbf");
		removeFile((inputRankDirStr + "/").c_str(), "prj");
