...
double singleGridSampleGeneration(int count, GeoGrid *geoGrid, double gridMinX, double gridMaxX, double gridMinY, double gridMaxY, OGRLayer* poLayer, OGRLayer* poLayer1, OGRLayer* poLayer2, char* outLabelDir, char* outImageDir, string strRank, OGRFeature* poDstFeature, GDALDriver* poDriver, OGRSpatialReference* poSpatialRef, ofstream& foutSample)
{
	...
	time_t gridRasterizeBegin, gridRasterizeEnd;
	time(&gridRasterizeBegin);
	...
	rasterize(sampleShpPathStr.c_str(), sampleImagePathStr.c_str());
  	...
  	time(&gridRasterizeEnd);
	double gridRasterizeCost = difftime(gridRasterizeEnd, gridRasterizeBegin);

	//foutSample << index << "\t" << polygonsNum1 << "\t" << polygonsNum2 << "\t" << verticesNum1 << "\t" << verticesNum2 << "\t" << varianceLayer1 << "\t" << varianceLayer2 << "\t" << avgDist << "\t" << referencePoint << "\t" << gridRasterizeCost << "\t" << gridCost << endl;
	return gridCost;
}
...
void parallelIntersectionSampleGeneration1(const char* inPath1, const char* inPath2, char* outLabelDir, char* outImageDir, int gridDimX, int gridDimY)
{
	...
	if (rank)
	{
		...
		int startGrid = (rank - 1) * averageGridNum;
		int endGrid = (rank - 1 + 1) * averageGridNum;
		if (rank == (size - 1)) endGrid = gridDimX * gridDimY;
		
		GeoGrid** geoGrid = new GeoGrid * [endGrid - startGrid];
		...
		for (int i = startGrid; i < endGrid; i++)
		{
			...
			int focalStep = 0;
			/*double focalSizeX = xSize / 100;
			double focalSizeY = ySize / 100;
			if (gridCost < 0.242) continue;
			if (gridCost >= 0.242 && gridCost < 0.484) focalStep = 1;
			if (gridCost >= 0.484 && gridCost < 0.605) focalStep = 2;
			if (gridCost >= 0.605 && gridCost < 0.726) focalStep = 3;
			if (gridCost >= 0.726 && gridCost < 0.847) focalStep = 4;
			if (gridCost >= 0.847 && gridCost < 0.968) focalStep = 5;
			if (gridCost >= 0.968) focalStep = 15;*/
			...
			for (int x = start; x <= end; x++)
			{
				...
				for (int y = start; y <= end; y++)
				{
					...
					/*singleGridSampleGeneration(count, gridMinX, gridMaxX, gridMinY, gridMaxY, sumRecord1, sumRecord2, poLayer, poLayer1, poLayer2, outLabelDir, outImageDir, strRank, poDstFeature, poDriver, poSpatialRef, foutSample);
					count += 1;*/
				}
